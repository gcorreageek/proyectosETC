...
function guardarIEE(){//objInformeInterno.cod_pedido  obs_informeinterno
	...
	$.post("guardarIEE",{"objInformeExterno.cod_ordencompra":codOrdenCompra,"objInformeExterno.obs_informeexterno":observacion},function(data){
		var bien = data.indexOf("Error");  
		if(bien<0){
			$("#idMensajeInterno").html(data);
	 		$('#myIE').modal({  keyboard: false });  
	 		setTimeout(function(){ $(location).attr('href','mainInformeExternoEntrada'); }, 4000);
		}else{
			$("#idMensajeInterno").html(data);
	 		$('#myIE').modal({  keyboard: false });  
		} 
